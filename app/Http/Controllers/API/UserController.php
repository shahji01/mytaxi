...
{
    public function updateDriverFaceRecognization(Request $request)
    {
        // Validate the incoming request
        $request->validate([
            'email' => 'required|email',
            'face_data' => 'required|string', // Ensure 'face_data' is provided and is a valid string
        ]);

        // Find the driver with the provided email and check if the user is of type 'driver'
        $driver = User::where('email', $request->email)
        ...
        if (!$driver) {
            return response()->json([
                'status' => 'error',
                'message' => 'Invalid driver credentials or user not found.',
            ], 404);
        }

        // Update the face recognition data for the driver
        $driver->face_data = $request->face_data;
        $driver->save();

        // Return a success response with the updated driver data
        return response()->json([
            'status' => 'success',
            'data' => $driver,
        ]);
    ...
    public function faceDetection(Request $request){
        ...
        $driver = User::where('email', $request->email)->where('user_type','driver')->first();
        if (!$driver) {
            return response()->json(['status' => 'error','message' => 'Invalid driver credentials or user not found.'], 404);
        }
        // Assuming face_data is sent for comparison
        if ($this->verifyFaceData($driver->face_data, $request->face_data)) {
            return response()->json(['status' => 'success','data' => 'Successfully Verified']);
        }

        return response()->json(['status' => 'error','message' => 'Face recognition failed'], 401);
    ...
    // Verify face data (you can use a custom algorithm for matching)
    private function verifyFaceData($storedFaceData, $incomingFaceData)
    {
        // Use some method to compare the stored and incoming face data
        return $storedFaceData === $incomingFaceData;
    }
	...
	public function UpdateUserDetail(Request $request)
    {
        ...
        $validatedData = $request->validate([
            ...
            'car_production_year'=> 'nullable|integer|min:1900|max:' . date('Y'),
            'work_address'       => 'nullable|string|max:255',
            'home_address'       => 'nullable|string|max:255',
            'work_latitude'      => 'nullable|numeric',
            'work_longitude'     => 'nullable|numeric',
        ...
        if ($user->userDetail) {
            ...
            $user->userDetail()->create($validatedData);
        }
        ...
        $user_detail = ($user->user_type == 'driver')
            ? new DriverResource($user)
            : new UserResource($user);

        // Prepare response
        $response = [
            'user' => $user_detail, // Single user object with userDetail included
        ];
