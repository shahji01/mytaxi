...
class HomeController extends Controller
{
    ...
    public function getAjaxList(Request $request)
    {
        ...
        switch ($request->type) {
            ...
            case 'document':
                $items = Document::with(['field:id,name'])->select('id','name','status' ,'is_required', 'has_expiry_date','document_field_id', DB::raw('(CASE WHEN is_required = 1 THEN CONCAT(name," * ") ELSE CONCAT(name,"") END) AS text'))->where('status',1);
                if($value != ''){
                    $items->where('name', 'LIKE', $value.'%');
                }
                $items = $items->get();
