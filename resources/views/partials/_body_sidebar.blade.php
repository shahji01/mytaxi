@php
    ...
    $MyNavBar = \Menu::make('MenuList', function ($menu) use($url){
        ...
        $menu->add('<span>'.__('message.location').'</span>', ['class' => ''])
            ->prepend('<i class="fas fa-user"></i>')
            ->nickname('location')
            ->data('permission', 'location list')
            ->link->attr(['class' => ''])
            ->href('#location');

            $menu->location->add('<span>'.__('message.list_form_title',['form' => __('message.location')]).'</span>', ['class' => 'sidebar-layout' ,'route' => 'location.index'])
                ->data('permission', 'location list')
                ->prepend('<i class="fas fa-list"></i>')
                ->link->attr(['class' => '']);

            $menu->location->add('<span>'.__('message.add_form_title',['form' => __('message.location')]).'</span>', ['class' => 'sidebar-layout' ,'route' => 'location.create'])
                ->data('permission', [ 'location add', 'location edit'])
